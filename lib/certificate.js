...
class CertificateManager {
  ...
  async issue(domain, options = {}) {
    ...
    // 通配符证书必须使用 DNS-01 验证
    if (domain.includes('*') && challengeType !== 'dns-01') {
      throw new Error(
        '通配符证书必须使用 DNS-01 验证。\n' +
        '请使用: --type dns-01 --dns-provider <provider>'
      );
    }
    ...
    if (challengeType === 'dns-01') {
      // DNS-01 验证支持本地模式，不需要 SSH
      console.log('使用 DNS-01 验证模式');
      return this.issueLocal(domain, options);
    }

    // HTTP-01 验证需要 SSH 远程模式
    if (!domainConfig.ssh || !domainConfig.ssh.host) {
      throw new Error(
        'HTTP-01 验证需要 SSH 远程服务器。请在 config/domains.yaml 中添加 ssh 配置:\n' +
        `${domain}:\n` +
        `  ssh:\n` +
        `    host: your-server.com\n` +
        `    remoteWebRoot: /var/www/html\n` +
        `    remoteNginxConfDir: /etc/nginx/conf.d\n` +
        `    remoteCertsDir: /etc/nginx/certs`
      );
    }
  ...
  /**
   * 本地模式申请证书（用于 DNS-01 验证）
   */
  async issueLocal(domain, options) {
    const client = await this.getClient();
    const challengeType = options.challengeType || this.config.challengeType;
    ...
    const challengeHandler = this.createChallengeHandler(challengeType, {
      ...
      dnsCredentials: options.dnsCredentials
    });
    ...
    // 生成 CSR（支持多域名）
    console.log('\n生成证书签名请求...');
    const altNames = domains.length > 1 ? domains.slice(1) : null;
    ...
    // 保存证书到本地
    const localPaths = await this.saveCertificate(domain, privateKey, cert);
  ...
  async issueRemote(domain, options, domainConfig) {
    ...
    const remoteWebRoot = (sshConfig.remoteWebRoot || '/var/www/html').replace(/\/$/, '');
  ...
  createChallengeHandler(type, options) {
    switch (type) {
      case 'http-01':
        // 本地 HTTP-01 处理器已不再使用，SSH 模式使用 Http01RemoteHandler
        throw new Error('HTTP-01 验证必须通过 SSH 远程模式执行');
  ...
  async saveDomainConfig(domain) {
    ...
    domains[domain] = {
      ...
      // 注意：不再自动添加 webRoot，如需指定请手动编辑 domains.yaml
    };
