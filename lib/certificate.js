...
class CertificateManager {
  ...
  async issue(domain, options = {}) {
    ...
    // 获取域名配置（用于获取该域名的独立 webRoot）
    const domainConfig = await this.getDomainConfig(domain);

    // 计算最终的 webRoot（优先级从高到低）
    const finalWebRoot = options.webRoot ||           // 1. 命令行参数
                         domainConfig.webRoot ||      // 2. 域名配置
                         this.config.webRoot ||       // 3. 全局配置
                         '/var/www/html';             // 4. 默认值
  ...
  async saveDomainConfig(domain, webRoot = null) {
    ...
    domains[domain] = {
      ...
      ...(webRoot && { webRoot }) // 如果有 webRoot 则保存
    };
