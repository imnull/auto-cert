...
class AutoCert {
  ...
  async deploy(domain, options = {}) {
    ...
    if (domainConfig.ssh) {
      // SSH 远程模式
      const sshUsername = domainConfig.ssh.username || 'root';
      console.log(`使用 SSH 远程部署模式: ${sshUsername}@${domainConfig.ssh.host}`);

      const sshClient = new SshClient({
        host: domainConfig.ssh.host,
        port: domainConfig.ssh.port || 22,
        username: sshUsername,
        privateKey: domainConfig.ssh.privateKey,
      ...
      try {
        // 去除末尾斜杠避免双斜杠
        const remoteWebRoot = (domainConfig.ssh.remoteWebRoot || domainConfig.webRoot || '').replace(/\/$/, '');
