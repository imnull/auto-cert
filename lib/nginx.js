...
`;
  ...
  async deployRemote(domain, options, sshClient) {
    ...
    const remoteConfigPath = path.posix.join(remoteNginxConfDir, `${domain}.conf`);

    console.log(`部署 nginx 配置到远程服务器...`);
    console.log(`  路径: ${remoteConfigPath}`);

    // 上传配置文件
    await sshClient.writeFile(remoteConfigPath, config);
