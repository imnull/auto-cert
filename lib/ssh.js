...
class SshClient {
  ...
  async connect() {
    return new Promise(async (resolve, reject) => {
      ...
      if (agentSock) {
        try {
          await fs.access(agentSock);
          console.log(`  检测到 SSH agent: ${agentSock}`);
        } catch {
          console.warn(`  警告: SSH_AUTH_SOCK 指向的 socket 不存在: ${agentSock}`);
        }
      }
      ...
      this.client.on('error', (err) => {
        console.error('  认证方法:', Object.keys(connectConfig).filter(k => ['password', 'privateKey', 'agent'].includes(k)));
      ...
      if (useAgent) {
        ...
        // 无论是否有 agent，都尝试加载默认私钥作为备用
        const defaultKeys = ['id_rsa', 'id_ed25519', 'id_ecdsa'];
        const home = require('os').homedir();
        
        for (const keyName of defaultKeys) {
          const keyPath = require('path').join(home, '.ssh', keyName);
          try {
            const keyContent = await fs.readFile(keyPath, 'utf8');
            // 如果没有 agent，使用私钥；有 agent 时私钥作为备用
            if (!connectConfig.agent && !connectConfig.privateKey) {
        ...
        if (!connectConfig.agent && !connectConfig.privateKey) {
          console.warn('警告: 未找到 SSH agent 或默认私钥');
          console.warn('请确保:');
          console.warn('  1. SSH agent 已运行: eval $(ssh-agent -s)');
          console.warn('  2. 密钥已添加: ssh-add ~/.ssh/id_rsa');
          console.warn('  3. 或在 domains.yaml 中指定 privateKey 路径');
        }
      }
