...
class SshClient {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (privateKey && !privateKey.includes('BEGIN')) {
        try {
          ...
          reject(new Error(`无法读取私钥: ${err.message}`));
          return;
        }
      }
  ...
  async installCertificate(domain, localCertDir, remoteCertDir) {
    ...
    // 上传证书文件
    const files = ['privkey.pem', 'cert.pem', 'chain.pem', 'fullchain.pem'];
    for (const file of files) {
      const localPath = path.join(localCertDir, file);
      const remotePath = path.posix.join(remoteDomainDir, file);
